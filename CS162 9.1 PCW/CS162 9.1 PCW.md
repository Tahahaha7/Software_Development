...
**Logging is the process that for every update that we make in a transaction we will record info to allow redo and undo in Log. Changes to data files must be written only after those changes have been logged. A transaction’s operations are provisional until "commit" outcome is logged to disk.**
