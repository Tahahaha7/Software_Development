...
-- 2. Write a transaction for a delivery from the Widge supplier which has just arrived at the ABC warehouse and unloaded 99 new Widgets.
BEGIN TRANSACTION;
...
-- 3. Write a transaction for a Customer order of 500 Widgets which places an order with the cheapest supplier.
BEGIN TRANSACTION;
INSERT INTO SupplierOrders VALUES (6003, 3002, [warehouseid], 500, "ORDERED", "2025-01-15", "2025-01-21");
END TRANSACTION;

-- 4. What statements would be needed to update a customers details to their new address, while still maintaining referential integrity?
BEGIN TRANSACTION;
UPDATE Customers SET AddressLine1 = "updated address" WHERE CustomerID=2001;
END TRANSACTION;

-- 5. What steps would we need to take to delete a product from the Product table, while still maintaining referential integrity?
BEGIN TRANSACTION;
DELETE FROM Product WHERE ProductID=3002;
DELETE FROM Inventory WHERE ProductID=3002;
END TRANSACTION;

-- 6.Critique the database design. What data should be in the database schema, but isn't?
/* 
In customer orders, we can add a column that specify if the delivery was completed.
It might be useful to add a product description in the SupplierProduct table
*/