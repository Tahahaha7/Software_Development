...
SELECT '----------------------------------------------------';
SELECT Drivers.NAME, COUNT(*) AS NUMBER_RIDES, SUM(Rides.COST) AS PAYMENT 
    FROM Rides
...
SELECT * FROM Clients
    ON Clients.CLIENTID = Rides.LIFTERNUMBER
    GROUP BY Rides.LIFTERNUMBER HAVING SUM(Rides.RIDENUMBER) IS NULL;